...
"""
High-level interface for the Attention MIL model family.

This module provides a stable entrypoint and re-exports public classes and
helpers from the modular codebase.
"""

from typing import Any

# Public API re-exports (stable surface)
...
def _run_hpo(args, outdir, num_workers, pin_memory, hpo):
    import shutil
    import optuna
    from .train.hpo import objective_mil_cv, save_study_artifacts, save_best_fold_metrics
...
def _run_final_with_best_if_needed(args, outdir, study, hpo, num_workers, pin_memory):
    if not args.export_leaderboard_attn:
        return
    from .utils.instances import load_and_merge_instances, build_instance_index
    from .train.hpo import train_best_and_export
