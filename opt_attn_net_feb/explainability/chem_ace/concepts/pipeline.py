...
class ChemACEPipeline:
    ...
    def generate_patches(self, *, molecules: Sequence[MoleculeSource]) -> list[PatchRecord]:
        ...
        logger.info("Generated patches", extra={"n_patches": len(all_patches)})
        return all_patches
    ...
    def tag_and_store_concepts(
        ...
        *,
        ...
        for cand in concept_set.candidates:
            cpatches = members_by_concept.get(cand.concept_local_id, [])
            if not cpatches:
                continue
            result = self.semantic_tagger.tag_concept(
                concept_id=cand.concept_local_id,
                concept_patches=cpatches,
                molecules_by_id=molecules_by_id,
            )
            self.repository.set_concept_label(concept_id=cand.concept_local_id, label_auto=result.label_auto)
            self.repository.upsert_tags(result.tags)
            out.append(result)
        logger.info("Tagged concepts", extra={"n_tagged": len(out)})
        return out
